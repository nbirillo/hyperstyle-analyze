...
class PlatformClient:
    ...
    def __init__(self, host: str, client_id: str, client_secret: str, port: Optional[int]):
        self.host = host
        self.client_id = client_id
        self.client_secret = client_secret
        if port is not None:
            self.token = self._get_authentication_code_token(port)
        else:
            self.token = self._get_client_credential_token()

    def _get_authentication_code_token(self, port: int):
        """ Runs authorization process using authentication-code grant type and
        gets session token for data exchange. """

        server = OauthServer(self.host, self.client_id, self.client_secret, port)
